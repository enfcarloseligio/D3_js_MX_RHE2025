...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Quintana Roo");
...
// ✅ Filtro de entidad para marcadores (Quintana Roo)
const ES_CVE_QUINTANA_ROO = "23";
const esDeQuintanaRooRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_QUINTANA_ROO
  || /quintana roo/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/quintana-roo.geojson"),
  d3.csv("../data/rate/quintana-roo.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Quintana Roo ${year}`;
  ...
  const filaTotal = tasasRaw.find(d =>
    String(d.id) === "9999" || (d.municipio || "").toUpperCase() === "TOTAL"
  );
  const totalEnt = filaTotal
    ? (Number(filaTotal.enfermeras_total ?? filaTotal.enfermeras) || 0)
  ...
  const projection = d3.geoMercator()
    .fitExtent([[20, 20], [MAP_WIDTH - 20, MAP_HEIGHT - 20]], {
  ...
  const zoom = d3.zoom()
    ...
    .on("zoom", (event) => {
      ...
      for (const ctl of ctlPorTipo.values()) {
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawQROO = raw.filter(esDeQuintanaRooRaw);
  ...
  generarTablaMunicipios("../data/rate/quintana-roo.csv");
  habilitarDescargaExcel("quintana-roo.xlsx");
  ...
  marcCtl.onChange(async () => {
    await updateMarcadores(marcCtl.getSelected());
  });
  ...
  document.getElementById("descargar-sin-etiquetas")?.addEventListener("click", () => {
    const titulo = construirTitulo(currentMetric, { entidad: "Quintana Roo", year });
    const etiquetas = document.getElementById("etiquetas-municipios");
    if (etiquetas) etiquetas.style.display = "none";
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-quintana-roo-sin-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-quintana-roo-con-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
...
}).catch(err => console.error("Error en mapa de Quintana Roo:", err));
