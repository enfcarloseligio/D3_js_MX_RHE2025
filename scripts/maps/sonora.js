...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Sonora");
...
// ✅ Filtro de entidad para marcadores (Sonora)
const ES_CVE_SONORA = "26";
const esDeSonoraRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_SONORA
  || /sonora/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/sonora.geojson"),
  d3.csv("../data/rate/sonora.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Sonora ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawSON = raw.filter(esDeSonoraRaw);
  ...
  generarTablaMunicipios("../data/rate/sonora.csv");
  habilitarDescargaExcel("sonora.xlsx");
...
}).catch(err => console.error("Error en mapa de Sonora:", err));
