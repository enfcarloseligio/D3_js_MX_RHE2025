...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Yucatán");
...
// ✅ Filtro de entidad para marcadores (Yucatán)
const ES_CVE_YUCATAN = "31";
const esDeYucatanRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_YUCATAN
  || /yucat[aá]n/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/yucatan.geojson"),
  d3.csv("../data/rate/yucatan.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Yucatán ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawYUC = raw.filter(esDeYucatanRaw);
  ...
  generarTablaMunicipios("../data/rate/yucatan.csv");
  habilitarDescargaExcel("yucatan.xlsx");
...
}).catch(err => console.error("Error en mapa de Yucatán:", err));
