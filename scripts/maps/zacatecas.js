...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Zacatecas");
...
// ✅ Filtro de entidad para marcadores (Zacatecas)
const ES_CVE_ZACATECAS = "32";
const esDeZacatecasRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_ZACATECAS
  || /zacatecas/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/zacatecas.geojson"),
  d3.csv("../data/rate/zacatecas.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Zacatecas ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawZAC = raw.filter(esDeZacatecasRaw);
  ...
  generarTablaMunicipios("../data/rate/zacatecas.csv");
  habilitarDescargaExcel("zacatecas.xlsx");
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-zacatecas-con-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
...
}).catch(err => console.error("Error en mapa de Zacatecas:", err));
