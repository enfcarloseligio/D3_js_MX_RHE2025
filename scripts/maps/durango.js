...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Durango");
...
// ✅ Filtro de entidad para marcadores (Durango)
const ES_CVE_DURANGO = "10";
const esDeDurangoRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_DURANGO
  || /durango/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/durango.geojson"),
  d3.csv("../data/rate/durango.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Durango ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawDGO = raw.filter(esDeDurangoRaw);
  ...
  generarTablaMunicipios("../data/rate/durango.csv");
  habilitarDescargaExcel("durango.xlsx");
...
}).catch(err => console.error("Error en mapa de Durango:", err));
