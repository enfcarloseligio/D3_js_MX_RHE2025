...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Veracruz");
...
// ✅ Filtro de entidad para marcadores (Veracruz)
const ES_CVE_VERACRUZ = "30";
const esDeVeracruzRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_VERACRUZ
  || /veracruz/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
Promise.all([
  ...
  document.title = `SIARHE | Enfermería en Veracruz ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawVER = raw.filter(esDeVeracruzRaw);
  ...
  generarTablaMunicipios("../data/rate/veracruz.csv");
  habilitarDescargaExcel("veracruz.xlsx");
...
}).catch(err => console.error("Error en mapa de Veracruz:", err));
