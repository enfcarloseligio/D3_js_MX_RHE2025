...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Jalisco");
...
// ✅ Filtro de entidad para marcadores (Jalisco)
const ES_CVE_JALISCO = "14";
const esDeJaliscoRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_JALISCO
  || /jalisco/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/jalisco.geojson"),
  d3.csv("../data/rate/jalisco.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Jalisco ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawJAL = raw.filter(esDeJaliscoRaw);
  ...
  generarTablaMunicipios("../data/rate/jalisco.csv");
  habilitarDescargaExcel("jalisco.xlsx");
...
}).catch(err => console.error("Error en mapa de Jalisco:", err));
