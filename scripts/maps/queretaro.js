...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Querétaro");
...
// ✅ Filtro de entidad para marcadores (Querétaro)
const ES_CVE_QUERETARO = "22";
const esDeQueretaroRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_QUERETARO
  || /queretaro|querétaro/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/queretaro.geojson"),
  d3.csv("../data/rate/queretaro.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Querétaro ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawQRO = raw.filter(esDeQueretaroRaw);
  ...
  generarTablaMunicipios("../data/rate/queretaro.csv");
  habilitarDescargaExcel("queretaro.xlsx");
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-queretaro-con-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
...
}).catch(err => console.error("Error en mapa de Querétaro:", err));
