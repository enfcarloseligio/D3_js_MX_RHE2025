...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Michoacán");
...
// ✅ Filtro de entidad para marcadores (Michoacán)
const ES_CVE_MICHOACAN = "16";
const esDeMichoacanRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_MICHOACAN
  || /michoacan|michoacán/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/michoacan.geojson"),
  d3.csv("../data/rate/michoacan.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Michoacán ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawMIC = raw.filter(esDeMichoacanRaw);
  ...
  generarTablaMunicipios("../data/rate/michoacan.csv");
  habilitarDescargaExcel("michoacan.xlsx");
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-michoacan-con-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
...
}).catch(err => console.error("Error en mapa de Michoacán:", err));
