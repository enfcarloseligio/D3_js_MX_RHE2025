...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Sinaloa");
...
// ✅ Filtro de entidad para marcadores (Sinaloa)
const ES_CVE_SINALOA = "25";
const esDeSinaloaRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_SINALOA
  || /sinaloa/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/sinaloa.geojson"),
  d3.csv("../data/rate/sinaloa.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Sinaloa ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawSIN = raw.filter(esDeSinaloaRaw);
  ...
  generarTablaMunicipios("../data/rate/sinaloa.csv");
  habilitarDescargaExcel("sinaloa.xlsx");
...
}).catch(err => console.error("Error en mapa de Sinaloa:", err));
