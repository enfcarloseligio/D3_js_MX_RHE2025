...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Puebla");
...
// ✅ Filtro de entidad para marcadores (Puebla)
const ES_CVE_PUEBLA = "21";
const esDePueblaRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_PUEBLA
  || /puebla/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/puebla.geojson"),
  d3.csv("../data/rate/puebla.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Puebla ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawPUE = raw.filter(esDePueblaRaw);
  ...
  generarTablaMunicipios("../data/rate/puebla.csv");
  habilitarDescargaExcel("puebla.xlsx");
...
}).catch(err => console.error("Error en mapa de Puebla:", err));
