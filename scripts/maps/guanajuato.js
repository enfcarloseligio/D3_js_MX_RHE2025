...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Guanajuato");
...
// ✅ Filtro de entidad para marcadores (Guanajuato)
const ES_CVE_GUANAJUATO = "11";
const esDeGuanajuatoRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_GUANAJUATO
  || /guanajuato/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/guanajuato.geojson"),
  d3.csv("../data/rate/guanajuato.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Guanajuato ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawGTO = raw.filter(esDeGuanajuatoRaw);
  ...
  generarTablaMunicipios("../data/rate/guanajuato.csv");
  habilitarDescargaExcel("guanajuato.xlsx");
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-guanajuato-con-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
...
}).catch(err => console.error("Error en mapa de Guanajuato:", err));
