...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Hidalgo");
...
// ✅ Filtro de entidad para marcadores (Hidalgo)
const ES_CVE_HIDALGO = "13";
const esDeHidalgoRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_HIDALGO
  || /hidalgo/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/hidalgo.geojson"),
  d3.csv("../data/rate/hidalgo.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Hidalgo ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawHGO = raw.filter(esDeHidalgoRaw);
  ...
  generarTablaMunicipios("../data/rate/hidalgo.csv");
  habilitarDescargaExcel("hidalgo.xlsx");
...
}).catch(err => console.error("Error en mapa de Hidalgo:", err));
