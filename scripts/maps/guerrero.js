...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Guerrero");
...
// ✅ Filtro de entidad para marcadores (Guerrero)
const ES_CVE_GUERRERO = "12";
const esDeGuerreroRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_GUERRERO
  || /guerrero/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/guerrero.geojson"),
  d3.csv("../data/rate/guerrero.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Guerrero ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawGRO = raw.filter(esDeGuerreroRaw);
  ...
  generarTablaMunicipios("../data/rate/guerrero.csv");
  habilitarDescargaExcel("guerrero.xlsx");
...
}).catch(err => console.error("Error en mapa de Guerrero:", err));
