...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Nayarit");
...
// ✅ Filtro de entidad para marcadores (Nayarit)
const ES_CVE_NAYARIT = "18";
const esDeNayaritRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_NAYARIT
  || /nayarit/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
Promise.all([
  ...
  document.title = `SIARHE | Enfermería en Nayarit ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawNAY = raw.filter(esDeNayaritRaw);
  ...
  habilitarDescargaExcel("nayarit.xlsx");
...
}).catch(err => console.error("Error en mapa de Nayarit:", err));
