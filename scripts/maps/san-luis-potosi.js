// scripts/maps/san-luis-potosi.js
// ==============================
// IMPORTACIONES
// ==============================
import {
...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – San Luis Potosí");
...
// ✅ Filtro de entidad para marcadores (San Luis Potosí)
const ES_CVE_SLP = "24";
const esDeSLPRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_SLP
  || /san luis potosi|san luis potosí/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/san-luis-potosi.geojson"),
  d3.csv("../data/rate/san-luis-potosi.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en San Luis Potosí ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawSLP = raw.filter(esDeSLPRaw);
  ...
  generarTablaMunicipios("../data/rate/san-luis-potosi.csv");
  habilitarDescargaExcel("san-luis-potosi.xlsx");
  ...
  document.getElementById("descargar-sin-etiquetas")?.addEventListener("click", () => {
    const titulo = construirTitulo(currentMetric, { entidad: "San Luis Potosí", year });
    const etiquetas = document.getElementById("etiquetas-municipios");
    if (etiquetas) etiquetas.style.display = "none";
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-san-luis-potosi-sin-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-san-luis-potosi-con-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
...
}).catch(err => console.error("Error en mapa de San Luis Potosí:", err));
