...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Coahuila");
...
// ✅ Filtro de entidad para marcadores (Coahuila)
const ES_CVE_COAHUILA = "05";
const esDeCoahuilaRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_COAHUILA
  || /coahuila/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/coahuila.geojson"),
  d3.csv("../data/rate/coahuila.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Coahuila ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawCOAH = raw.filter(esDeCoahuilaRaw);
  ...
  generarTablaMunicipios("../data/rate/coahuila.csv");
  habilitarDescargaExcel("coahuila.xlsx");
  ...
  document.getElementById("descargar-sin-etiquetas")?.addEventListener("click", () => {
    const titulo = construirTitulo(currentMetric, { entidad: "Coahuila", year });
    const etiquetas = document.getElementById("etiquetas-municipios");
    if (etiquetas) etiquetas.style.display = "none";
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg",
        "mapa-coahuila-sin-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
...
}).catch(err => console.error("Error en mapa de Coahuila:", err));
