...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Nuevo León");
...
// ✅ Filtro de entidad para marcadores (Nuevo León)
const ES_CVE_NUEVO_LEON = "19";
const esDeNuevoLeonRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_NUEVO_LEON
  || /nuevo leon|nuevo león/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/nuevo-leon.geojson"),
  d3.csv("../data/rate/nuevo-leon.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Nuevo León ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawNLE = raw.filter(esDeNuevoLeonRaw);
  ...
  generarTablaMunicipios("../data/rate/nuevo-leon.csv");
  habilitarDescargaExcel("nuevo-leon.xlsx");
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG(
        "#mapa-entidad svg",
        "mapa-nuevo-leon-con-nombres.png",
        MAP_WIDTH,
        MAP_HEIGHT,
        { titulo }
      );
...
}).catch(err => console.error("Error en mapa de Nuevo León:", err));
