...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Tlaxcala");
...
// ✅ Filtro de entidad para marcadores (Tlaxcala)
const ES_CVE_TLAXCALA = "29";
const esDeTlaxcalaRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_TLAXCALA
  || /tlaxcala/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/tlaxcala.geojson"),
  d3.csv("../data/rate/tlaxcala.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Tlaxcala ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawTLAX = raw.filter(esDeTlaxcalaRaw);
  ...
  generarTablaMunicipios("../data/rate/tlaxcala.csv");
  habilitarDescargaExcel("tlaxcala.xlsx");
...
}).catch(err => console.error("Error en mapa de Tlaxcala:", err));
