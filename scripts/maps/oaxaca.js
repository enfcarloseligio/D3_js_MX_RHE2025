...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Oaxaca");
...
// ✅ Filtro de entidad para marcadores (Oaxaca)
const ES_CVE_OAXACA = "20";
const esDeOaxacaRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_OAXACA
  || /oaxaca/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/oaxaca.geojson"),
  d3.csv("../data/rate/oaxaca.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Oaxaca ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawOAX = raw.filter(esDeOaxacaRaw);
  ...
  generarTablaMunicipios("../data/rate/oaxaca.csv");
  habilitarDescargaExcel("oaxaca.xlsx");
...
}).catch(err => console.error("Error en mapa de Oaxaca:", err));
