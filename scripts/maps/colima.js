...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Colima");
...
// ✅ Filtro de entidad para marcadores (Colima)
const ES_CVE_COLIMA = "06";
const esDeColimaRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_COLIMA
  || /colima/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/colima.geojson"),
  d3.csv("../data/rate/colima.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Colima ${year}`;
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawCOL = raw.filter(esDeColimaRaw);
  ...
  generarTablaMunicipios("../data/rate/colima.csv");
  habilitarDescargaExcel("colima.xlsx");
...
}).catch(err => console.error("Error en mapa de Colima:", err));
